...
char* read()
{
	FILE* fp = fopen("C:\\Users\\周波\\Desktop\\color.exe", "rb");
	if (fp == NULL)
	{
...
void cunpan(char* file)
{
	...
	FILE* fp = fopen("C:\\Users\\周波\\Desktop\\color2.exe", "wb");
	if (fp == NULL)
	{
		printf("File open failed.\n");
		exit(1);
...
void IMGconvertFILE(char* img,char* file)
{
	...
	//先从img开始，把pe头部分减掉，在节里面找数据  
	PIMAGE_SECTION_HEADER imgjie = (PIMAGE_SECTION_HEADER)pSectionH[0];
	unsigned int imgdata1 = ((unsigned int)img + imgjie->VirtualAddress +10) - (unsigned int)img;//距离头的位置
	unsigned int jie1;
	for (int i = 0; i < pPEHeader->NumberOfSections;i++)
	{
	...
	unsigned int fike1;
	unsigned int filedata1 = ((unsigned int)file + filejie->PointerToRawData + 10) - (unsigned int)file;//距离头的位置
	...
	//img+节里面某个数据 假设img=1000，节里面数据在1500，1500-1000=500，
